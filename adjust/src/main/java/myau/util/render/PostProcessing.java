...
import myau.util.shader.BloomShader;
...
public class PostProcessing {
    ...
    public static void drawBlur(float x, float y, float x2, float y2, Supplier<Runnable> maskDrawer) {
        ...
        if (!myau.module.modules.PostProcessing.isBlurEnabled()) {
            return;
        }
        ...
        float strength = myau.module.modules.PostProcessing.getBlurStrength();
    ...
    public static void drawBlurRect(float x, float y, float x2, float y2) {
        drawBlur(x, y, x2, y2, () -> () -> {
            ...
            GlStateManager.tryBlendFuncSeparate(770, 771, 1, 0);
            myau.util.render.RenderUtil.setup2DRendering(() -> {
    ...
    public static Framebuffer beginBloom() {
        if (!myau.module.modules.PostProcessing.isBloomEnabled()) return null;
    ...
    public static void endBloom(Framebuffer bloomBuffer) {
        ...
        BloomShader.renderBloom(bloomBuffer.framebufferTexture, myau.module.modules.PostProcessing.getBloomIterations(), myau.module.modules.PostProcessing.getBloomOffset());
    }

    public static void endBloomOutside(Framebuffer bloomBuffer, Runnable outsideMask) {
        if (bloomBuffer == null) return;
        mc.getFramebuffer().bindFramebuffer(false);
        BloomShader.renderBloomOutside(bloomBuffer.framebufferTexture, myau.module.modules.PostProcessing.getBloomIterations(), myau.module.modules.PostProcessing.getBloomOffset(), outsideMask);
    }
}
