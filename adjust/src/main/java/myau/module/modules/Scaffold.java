...
public class Scaffold extends Module {
    ...
    private FontRenderer getBlockCounterFont(HUD hud) {
        return hud != null ? hud.getArrayListFontRenderer() : mc.fontRendererObj;
    }

    private void drawBlockCounter(int count) {
        HUD hud = (HUD) Myau.moduleManager.modules.get(HUD.class);
        FontRenderer font = getBlockCounterFont(hud);
        ScaledResolution sr = new ScaledResolution(mc);
        String countText = String.valueOf(count);
        String boldCountText = "\u00a7l" + countText + "\u00a7r";
        String blocksText = "blocks";
        float scale = hud != null ? hud.scale.getValue() : 1.0F;
        float gap = 3.0F;
        int countWidth = font.getStringWidth(boldCountText);
        int totalWidth = Math.round(countWidth + gap + font.getStringWidth(blocksText));
        float x = sr.getScaledWidth() / 2.0F - totalWidth / 2.0F;
        float y = sr.getScaledHeight() / 2.0F + 18.0F;
        int countColor = hud != null ? hud.getCustomColor1().getRGB() : Color.WHITE.getRGB();
        boolean textShadow = hud != null && hud.shadow.getValue();
        ...
        font.drawString(boldCountText, scaledX, scaledY, countColor, textShadow);
        font.drawString(blocksText, scaledX + countWidth + gap, scaledY, Color.WHITE.getRGB(), textShadow);
    ...
    public void onRender(Render2DEvent event) {
        if (this.isEnabled()) {
            if (this.blockCounter.getValue()) {
                drawBlockCounter(getHotbarBlockCount());
            }
        }
    }
